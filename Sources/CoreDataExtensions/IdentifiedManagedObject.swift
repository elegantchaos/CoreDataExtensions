...
public protocol IdentifiedManagedObject where Self: NSManagedObject, Self: Identifiable {
    var id: Self.ID { get set }
    static func arg(for id: Self.ID) -> CVarArg
...
public extension IdentifiedManagedObject {
    ...
    static func withID(_ id: Self.ID, in context: NSManagedObjectContext, createIfMissing: Bool = false) -> Self? {
        ...
        request.predicate = NSPredicate(format: "id = %@", arg(for: id))
    ...
    static func arg(for id: Self.ID) -> CVarArg where ID: CVarArg {
        id
    }
}
